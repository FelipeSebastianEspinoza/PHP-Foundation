...
<html class="no-js" lang="en" dir="ltr">
  <head>
    ...
    <title>Historial de Actividades y Mutual</title> 
    ...
    <div class="off-canvas-content" data-off-canvas-content>
        <div class="grid-x grid-padding-x">
            ...
            <div class="row column">
                <hr>
                <h4 style="margin: 0;" class="text-center">Historial de Actividades y Mutual</h4>
                <hr>
            </div>
            <div class="callout">
...
<tbody>
  <?php  
    ...
    $result = mysqli_query($conn, 'SELECT *
      ...
      echo '<input type="hidden" name="id_historialmutual" value='.$row["id_historialmutual"].' />';
...
<tr>  
 ...
 <input type="hidden" name="id_historialmutual" value="<?php echo $id_historialmutual ?>"/> 	
...
<?php
 $result = mysqli_query($conn, 'SELECT *
							    FROM mutual_anexos
								WHERE eliminar!="1"
								AND id_historialmutual='.$id_historialmutual.';');
		    while($row = mysqli_fetch_array($result)){ 
?> 
<table>
  <tbody> 
<tr>  
        ...
        <form class="formulario" action="" method="post" id="usrform" enctype="multipart/form-data"> 
        <input type="hidden" name="id_mutualanexos" value="<?php echo $row["id_mutualanexos"] ?>"/> 		
        <input type="hidden" name="id_historialmutual" value="<?php echo $id_historialmutual?>"/>
...
if(isset($_POST['submitmodificarriesgo'])){
    ...
    $nuevo->ModificarHistorialMutual($campos);
}
...
if(isset($_POST['submitarchivomodificar'])){
    ...
    $campos = array("id_historialmutual"=> $_POST['id_historialmutual'],"id_mutualanexos"=> $_POST['id_mutualanexos'],
    "nombre"=> $_POST['nombre'] ); 
