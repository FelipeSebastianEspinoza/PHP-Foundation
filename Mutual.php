...
?>	
 <table>
  <thead>
	...
	<?php  
	  echo'<form class="formulario" action="MMutual.php" method="post" id="usrform" enctype="multipart/form-data">';
...
<?php  $id_unidad2=$row["id_historialmutual"];

            $result2 = mysqli_query($conn, 'SELECT *
								    FROM mutual_anexos  
								    WHERE '.$id_unidad2.'=id_historialmutual 
									AND eliminar!="1"
										   ;');
...
if(isset($_POST['submitprotocolo'])){
    ...
    $campos = array("id_historialmutual"=> NULL ,
	"titulo"=>$_POST['titulo'],
	"fecha"=>$_POST['fecha'],
    ...
    $nuevo->NuevoHistorialMutual($campos);
}
if(isset($_POST['eliminarenfermedades'])){
    ...
    $nuevo = new GuardarHistorialMutual("tesis"); 
    $nuevo->EliminarHistorialMutual($campos);
}
